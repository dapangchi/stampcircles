<?php
  //Simple Example with One set of Data Line Graph
	...
	$graph->Graph->setSubtype('map');   //chart or axis
	$graph->printGraph();

	
	//Graph
	$graph->Graph->setType('line');       //line bar pie venn scatter
	$graph->Graph->setSubtype('axis');   //chart or axis
	$graph->Graph->setSize(350, 350);     //size in pixels
	$graph->Graph->setAxis(array('x','t','y')); //no arguments means all on
	$graph->Graph->setGridLines(5, 5, 3, 2);  //x axis y axis length of line length of blank
	                                          //effectively sets the grid lines ie the graph paper
	
	//Title
	$graph->Graph->setTitle('Line Chart', '#bebebe', 18);	  //sets title 
	...
	$graph->Graph->setAxisRange(array(0, 200, 0, 200));
	$graph->Graph->setAxisRange(array(0, 200, 0, 200));
	$graph->Graph->setAxisRange(array(0, 200, 0, 200));
