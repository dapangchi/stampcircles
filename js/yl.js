...
UI.console.addButtons=function(){
    $('<button>Edit</button>').insertAfter('.eval').addClass('edit').attr("disabled", "true").css('background-color','#999');
...
UI.console.addJSLint=function(code){
    ...
    var heading='<h5>jslint Report: </h5>';
    // insert jslint
  ...
  return '<div class="floating_notice clearfix">'+
        '<div class="floating_notice_top" style="background:url(http://localhost/codeigniter/images/floating_notice_top.png);'
                ...
                +'<p id="floating-notice-message" style="width:90%;font-family:inherit;\n\
                   ...
                   +'<img src="/CodeIgniter/images/underground_thumb.jpg" style="width:35px;float:left;margin-right:8px;margin-top:0">'
                   +msg
                   +'</p>'
                +'</div>'
...
*/
   ...
   // var DEBUG = true;
    ...
    window.INFO =INFO;
   // UI.formatCode('code');
    UI.floatingNotices('You can earn points by runnning the examples!');
...
window.morph= function (){
   ...
   $('#pagehead-spacer').height('30px');
   $('#edit-block').hide();
};

window.unmorph = function (){
   $('#left-menu').css({'visibility':'visible'});
   $('#left-menu').width('250px');
   $('#left-menu').height('auto');
   $('#pagehead-spacer').height('80px');
   $('#edit-block').show();
