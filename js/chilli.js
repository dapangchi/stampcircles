...
for (i=1;i<10;i++){
var z='<div style="background-color:rgba(255,0,0,opacity);height:50px;width:50px;float:left"></div>';
$(this).append(z);
...
$('#colors').add('<table cellspacing="0" id="grid">');
var i=0;
for (i = 5; i >= 0; i--) {
	for (j = 5; j >= 0; j--) {
		$(this).add('<tr>');
...
for (i = 5; i >= 0; i--) {
	for (j = 5; j >= 0; j--) {
		s=s+'<div class="row">';
		for (k= 5; k >= 0; k--) {
			 col = colors[i]+colors[j]+colors[k];
			s=s+'<div class="col" style="background: #'+col+';"></div>';
		}
		s=s+'</div>';
	}
}
...
Array.prototype.forEach=function(f){
    var i=this.length,j,l=this.length;for(i=0;i<l;i++){
        if((j=this[i])){
            f(j);
        }
        }
...
Array.prototype.insert=function(i,v){
    if(i>=0){
        var a=this.slice(),b=a.splice(i);a[i]=value;return a.concat(b);
    }
    }
Array.prototype.shuffle=function(){
    var i=this.length,j,t;while(i--){
        j=Math.floor((i+1)*Math.random());t=arr[i];arr[i]=arr[j];arr[j]=t;
    }
    }
Array.prototype.unique=function(){
    var a=[],i;this.sort();for(i=0;i<this.length;i++){
        if(this[i]!==this[i+1]){
            a[a.length]=this[i];
        }
        }return a;
...
if(typeof Array.prototype.splice==='undefined'){
    Array.prototype.splice=function(a,c){
        var i=0,e=arguments,d=this.copy(),f=a;if(!c){
            c=this.length-a;
...
if(typeof Array.prototype.unshift==='undefined'){
    Array.prototype.unshift=function(a){
        this.reverse();var b=this.push(a);this.reverse();return b;
    };
}
