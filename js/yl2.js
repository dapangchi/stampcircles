// insert all buttons
// refactor for other consoles also
// need to add textarea here
UI.console.addPHPButtons=function(baseElementClass){
    $('<button>Edit</button>').insertAfter(baseElementClass).addClass('editPHP').attr("disabled", "true").css('background-color','#999');
    $('<button>Clear</button>').insertAfter('.editPHP').addClass('clr').hide();
    $('<button>Done</button>').insertAfter('.clr').addClass('donePHP').hide();
}
...
var s='<form class="myForm1" action="" method="post" style="padding-top:0;padding-bottom:0;margin-bottom:0;margin-top:0">'
      +'<textarea name="ascript" value="test" class="PHPTextArea"  style="display:none">'
...
$('code').each(function(){
    ...
    var $phpTextArea= grandparent.find('.PHPTextArea');
        $phpTextArea.val(c);
 });
   ...
   // UI.formatCode('code');
    UI.floatingNotices('Welcome to PHP!<br/>Watch your $!');
    // buttons with base class
    // at this stage we add all buttons
    UI.console.addPHPButtons('.evalPHP');


   
    // Main evaluation of code
    $('button.evalPHP').live('click', function(e){
        UI.showPoints($(this));
        //outside try to enable edit key in case of error
        $that=$(this);
        log('... running ');
        // enable edit key
        $that.siblings('.editPHP').css('background-color','#666').attr('disabled','');
    ...
    $('.editPHP').live('click', function () {
        ...
        var par=$(this).parent();
        var $phpTextArea=par.find('.PHPTextArea');
        
        var phpCodeElement=par.find('code');
            phpCodeElement.hide();
            //alert(phpCodeElement.text());
        var theCode=phpCodeElement.text();
            $phpTextArea.val(theCode).show();
            
        var $done=$edit.siblings('.donePHP').show();
  ...
  $('.donePHP').live('click', function () {
        //UI.floatingNotices('You need to press Re-run to execute your revision!');
        //read the value  from  the text area
        var par=$(this).parent();
        var $phpTextArea= par.find('.PHPTextArea');
        var newCode = $phpTextArea.val();
            $phpTextArea.hide();
            log('new code',newCode);
            var phpCodeElement=par.find('code');
            phpCodeElement.text(newCode);
            phpCodeElement.show();

        $(this).siblings('.evalPHP').show();
        $(this).siblings('.editPHP').show();
     ...
     *
     */
    UI.evalPHP= function (target,theForm, url){
        var options = {
            target: target,
            ...
            url: '/codeigniter/sandbox/haskell/?',
            type: 'POST'
