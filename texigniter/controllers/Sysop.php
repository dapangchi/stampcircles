...
class Sysop extends CI_Controller {
	...
	function paths () {
	  echo_array($_SERVER);
	  echoPRE($this->config->item('base_url'));
	 
	  //domain with no slashes
	  preg_match('/\/('.$this->config['domain_name'].')\//', $this->config->item('base_url'), $matches);
	  
	  echoPRE($matches[1]); 
	  echoPRE(site_url());
	  //echoPRE($this->config['base_url'] == '');
	  
	  $domain_name = str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
	  $domain_name = str_replace('/', '', $domain_name);
	  echoPRE('BASE URL: '.$domain_name);
     ...
     *  Some test for encyption and keys
     */
    function encrypt() {
        $s = 'test';
        $textToEncrypt = "yiannis lazarides";
        $encryptionMethod = "AES-256-CBC";  // AES is used by the U.S. gov't to encrypt top secret documents.
        $secretHash = "25c6c7ff35b9979b151f2136cd13b0yy";
        //$iv_size = mcrypt_get_iv_size('des', 'ecb');
        $iv = 'yiannislazarides' ;//mcrypt_create_iv($iv_size, MCRYPT_RAND);
        
        //To encrypt
        $encryptedMessage = base64_encode(openssl_encrypt($textToEncrypt, $encryptionMethod, $secretHash, 0, $iv));

        //To Decrypt
        $decryptedMessage = openssl_decrypt(base64_decode($encryptedMessage), $encryptionMethod, $secretHash, 0, $iv);
...
<ul id="LHnav" style="margin-left:0px;width:210px">
  ...
  <li><a href="../'.$dir.'/ ">...</a></li>
    ...
    private function save_post_data($src, $content){
        ...
        {
            //echo '...File written! to '."FCPATH.$portal.'/'.$dir.'/'.$file_name.'.dat';
        }
    }
