...
\batchmode
 ...
 \usepackage{mathpazo}
\usepackage[T1]{fontenc}  % this order is good?
\usepackage[utf8]{inputenc}
...
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{wasysym} % some conflicts with amsmath

\usepackage{alltt}
\usepackage{verbatimbox} % centers verbatim text
\let\ETH\DH % capital letter Eth
...
\DeclareRobustCommand\captionlorem{Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
      Sed nibh justo,  et blandit lorem.}
...
% Needs development to get where it should be.
% minipage needs to be a key value store
\def\phl[width = #1\textwidth]#2#3{%
\fboxsep3pt%
\fboxrule0pt%
\noindent
...
\def\covertext{This is a test}
\IfFileExists{../[[country]]/cover-image.jpg}{
    \def\coverpicture{../[[country]]/cover-image}}%
{\def\coverpicture{../../../tex-templates/cover-image}%
}

\IfFileExists{../cover-text.dat}{
    \gdef\covertext{\input{../cover-text.dat}}}%
{\gdef\covertext{This cover image shows Jo Bodeon, a back-roper in the mule room at Chace 
...
\let\oldtextcent\textcent

\def\texcent{TT\oldtextcent TT}
...
%London Philatetelist
\def\LP#1#2#3#4{#1, \textit{#2}, #3, #4 }

\def\soldp#1#2{\pound#1 sold at \pound#2}

\def\ilink#1#2{\url{#2}}

\def\tete{t\^et\^e beche }

\def\clear{}

\def\karpov{\textit{ex} Anatoly Karpov, 2012.}

\def\dudley{ex Dudley. }
...
\newenvironment{letter}{\begin{quotation}}{\end{quotation}}

\def\SignatureR#1{\hfill#1}

\long\def\closing#1{\leavevmode\par \hfill\hbox to 4cm{\vbox{#1}}\par }
